...
for _, row in data_layer.get_raw_recipes().iterrows():
    ...
    """, (row['id'], row['submitted'], row['name'], row['minutes'], row['description'], row['n_steps'], row['steps'], row['n_ingredients']))
conn.commit()
print("ok recette")
...
ingredient_cache = {row[1]: row[0]
                    for row in cur.fetchall()}  # {name: ingredient_id}
