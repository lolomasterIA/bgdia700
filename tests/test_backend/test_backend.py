...
def test_top_ingredient_used(session, mock_data):
    ...
    # Test case for data_type == "One word"
    result = backend.top_ingredient_used(session, 2, data_type="One word")
    assert isinstance(result, list)
    assert len(result) == 2
    assert result[0][0] == "Ingredient 2"

    # Test case for data_type != "One word"
    result = backend.top_ingredient_used(session, 2, data_type="Multiple words")
    assert isinstance(result, list)
    assert len(result) == 2
    assert result[0][0] == "Ingredient 2"
...
def test_top_ingredient_rating(session, mock_data):
    ...
    # Test case for data_type != "One word"
    avg_rating, review_count = backend.top_ingredient_rating(
        session, data_type="Multiple words"
    )
    assert isinstance(avg_rating, dict)
    assert isinstance(review_count, dict)
    assert review_count["Ingredient 2"] == 20
...
def test_generate_kmeans_ingredient(session, mock_data):
    ...
    # Test case for data_type != "One word"
    reduced_data, all_ingredients, kmeans = backend.generate_kmeans_ingredient(
        session, 2, data_type="Multiple words"
    )
    assert isinstance(reduced_data, np.ndarray)
    assert isinstance(all_ingredients, list)
    assert len(all_ingredients) == 3
...
def test_get_ingredient_rating(session):
    ...
    mock_query.first.return_value = Rating(rating=expected_rating)  # Simule le résultat

    # Test case for data_type == "One word"
    result = backend.get_ingredient_rating(
        session, ingredient_name, data_type="One word"
    )
    ...
    # Test case for data_type != "One word"
    result = backend.get_ingredient_rating(
        session, ingredient_name, data_type="Multiple words"
    )
    assert isinstance(result, float)
    assert result == 4.5
...
def test_nb_ingredient(session):
    # Mock la requête SQLAlchemy
    mock_query = MagicMock()
    session.query.return_value = mock_query
    mock_query.where.return_value = mock_query  # Simule le chaînage
    mock_query.scalar.return_value = 10  # Simule le résultat

    # Test case for ingredient_data_type == "One word"
    result = backend.nb_ingredient(session, ingredient_data_type="One word")
    assert isinstance(result, int)
    assert result == 10

    # Test case for ingredient_data_type != "One word"
    result = backend.nb_ingredient(session, ingredient_data_type="Multiple words")
    assert isinstance(result, int)
    assert result == 10
